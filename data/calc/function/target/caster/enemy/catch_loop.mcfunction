#> calc:target/caster/enemy/catch_loop
### 遠隔攻撃ターゲットを取得するサブルーチン

# 残り深さをデクリメント
execute store result storage calc: Depth int 1 run data get storage calc: Depth 0.9999999999
# 探索
execute positioned ^ ^ ^5 as @e[distance=..5,tag=Enemy] positioned as @s positioned ^ ^ ^2000 facing entity 0-0-0-0-0 feet positioned ^ ^ ^2000 positioned ~-0.5 ~-0.5 ~-0.5 run tag @s[dx=0,dy=0,dz=0] add CasterTargetCand
