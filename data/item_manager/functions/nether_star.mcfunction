...
execute if score $PlayerCount Global matches 1 run tellraw @a ["",{"selector":"@a[sort=nearest]"},"は",{"score":{"name":"$Devidend","objective":"Global"}},"経験値を得た！"]
execute if score $PlayerCount Global matches 2.. run tellraw @a ["",{"selector":"@a[sort=nearest]"},"達はネザースターを分け合い、それぞれ",{"score":{"name":"$Devidend","objective":"Global"}},"経験値を得た！"]
execute if score プール Exp matches 1.. run tellraw @a ["",{"score":{"name":"プール","objective":"Exp"}},"個のネザースターが持ち越された。"]
