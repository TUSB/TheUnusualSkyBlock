...
execute if score $summoned Global matches 0 if score $cleared Global matches 1 store success score $summoned Global run summon minecraft:item ~ ~ ~ {Tags:[inf_item],Item:{id:"minecraft:drowned_spawn_egg",Count:1b,tag:{display:{Name:"\"§fサポートアクション§bサファイア\""},Enchantments:[{id:""}],EntityTag:{DeathLootTable:"minecraft:empty",IsBaby:true,Tags:[Egg,Initialized,NativeTask,ObserveCooldown,CooldownRequired],PortalCooldown:1,ActiveEffects:[{Id:14b,Duration:1000,ShowParticles:false}],DeathTime:19s,Silent:true,HandItems:[],ArmorItems:[]}}}}
