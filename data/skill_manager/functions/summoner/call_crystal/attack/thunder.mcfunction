...
scoreboard players operation @e[distance=..15,tag=Mob,tag=!WeakToThunder,nbt={OnGround:true}] Damage += $BaseDamage Global
scoreboard players operation $BaseDamage Global *= $2 Const
scoreboard players operation @e[distance=..15,tag=Mob,tag=!WeakToThunder,nbt=!{OnGround:false}] Damage += $BaseDamage Global
