...
particle minecraft:dust 1 0.78125 0 3 ~ ~-1 ~ 5 0 5 0 100 force
###---演出---End

function skill_manager:summoner/call_crystal/attack/common

scoreboard players operation @e[distance=..15,tag=Mob,tag=!WeakToEarth,nbt={OnGround:false}] Damage += $BaseDamage Global
scoreboard players operation $BaseDamage Global *= $2 Const
scoreboard players operation @e[distance=..15,tag=Mob,tag=!WeakToEarth,nbt={OnGround:true}] Damage += $BaseDamage Global
