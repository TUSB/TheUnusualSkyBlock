...
###弾の設定をする
execute positioned ^ ^ ^-0.25 run summon minecraft:snowball ~ ~1.52 ~ {NoGravity:true,Color:-1,Tags:[FlyingRequired,Projectile,Initializing,Rapid,NativeTask,AerialSlash]}
...
execute if score @s ModeSkill matches 11041 run scoreboard players set $Damage Global 160000
execute if score @s ModeSkill matches 11042 run scoreboard players set $Damage Global 320000
execute if score @s ModeSkill matches 11043 run scoreboard players set $Damage Global 580000
execute if score @s ModeSkill matches 11044 run scoreboard players set $Damage Global 2300000
...
execute as @e[tag=CasterTarget,limit=1] run function calc_manager:set/id
scoreboard players operation @e[tag=Initializing,limit=1] ID = @e[tag=CasterTarget,limit=1] ID
