...
#ダメージを受けてHurtByTimestampが更新されると0に
execute unless entity @s[nbt={HurtByTimestamp:0}] run scoreboard players set @s SleepgaTime 0
data merge entity @s {HurtByTimestamp:0}

execute if score @s SleepgaTime matches 0 run data merge entity @s {NoAI:0b}
