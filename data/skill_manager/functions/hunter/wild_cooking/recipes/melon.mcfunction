...
scoreboard players operation $Threshold Global += $Modifier Global
execute if score $Give Global matches 0 if score $Threshold Global matches 0.. run give @s minecraft:potion{display:{Name:"[\"§c§lスイカジュース\"]",Lore:["§rあまくて さっぱりした ジュース"]},CustomPotionColor:13458524,CustomPotionEffects:[{Id:14b,Duration:1,Amplifier:51b,ShowParticles:false}]} 1
execute if score $Give Global matches 0 if score $Threshold Global matches 0.. run function skill_manager:hunter/wild_cooking/direction/success
