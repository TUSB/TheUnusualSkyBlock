#> item:sign/click/harvest_wheat
# 範囲内の育ち切った作物の数を記録
execute store result score _ Calc store result score _ _ run clone ~-9 ~-1 ~-9 ~9 ~7 ~9 ~-9 ~-1 ~-9 filtered minecraft:wheat[age=7] force
...
# 得られた種の数を取得
execute in area:control_area positioned 5.0 5.0 5.0 run tag @e[distance=0,nbt={Item:{id:"minecraft:wheat_seeds"}}] add ConsumeItem
...
function makeup:item/sign/common_firework.macro {color:9017865}
