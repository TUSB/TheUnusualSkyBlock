...
#上限を超えないようにバンドルに移す
data modify storage item: JewelBundle.Bundle.tag.Items append from storage item: JewelBundle.Items[0]
execute store result score _ _ run data get storage item: JewelBundle.Items[0].Count
execute if score # Calc < _ _ store result storage item: JewelBundle.Bundle.tag.Items[-1].Count byte 1 run scoreboard players get # Calc
