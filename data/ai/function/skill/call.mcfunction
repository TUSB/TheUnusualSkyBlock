...
execute unless score # MP > @s MP run function ai:call/execute/
execute unless score # MP > @s MP run scoreboard players operation @s MP -= # MP
#Interval適用
execute store result storage mob_data: AI.Turn[0].Skill[0].Interval.Current int 1 run function ai:skill/set_interval
#Loop削減 0のときNext処理
...
execute if data storage mob_data: AI.Turn[0].Skill[0].Loop{Current:0} run function ai:skill/skip
