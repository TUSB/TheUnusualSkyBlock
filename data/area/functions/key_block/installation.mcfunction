...
# 表示するアイテムを指定する
execute at @s run data modify entity @e[tag=KeyBlock.Block,distance=..1.5,limit=1] item set from storage area: key_block.item
# アイテムの名前を表示する
execute at @s run data modify entity @e[tag=KeyBlock.Block,distance=..1.5,limit=1] CustomName set from storage area: key_block.item.tag.display.Name
# 向きを記録
execute at @s run data modify entity @e[tag=KeyBlock.Key,distance=..1.5,limit=1] Rotation set from storage area: key_block.Rotation
