...
scoreboard players set @s[gamemode=!creative,gamemode=!spectator] DoomCount 31
execute if entity @s[gamemode=!creative,gamemode=!spectator] run tellraw @a [{"text":"","color":"red"},{"selector":"@s"},"は",{"text":"死の宣告","color":"white","hoverEvent":{"action":"show_text","value":"一定時間経過後、死亡する。","color":"white"}},"を受けた！"]
