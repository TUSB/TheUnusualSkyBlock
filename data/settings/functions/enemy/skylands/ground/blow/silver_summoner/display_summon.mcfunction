...
# シルバー召喚
    ...
    execute if block ~ ~ ~ #main:no_collision as @e[tag=Spawn,tag=!CallSpawned,distance=0] run function skill:enemy/spawn/apply
    execute unless block ~ ~ ~ #main:no_collision as @e[tag=Spawn,tag=!CallSpawned,distance=0] run function settings:enemy/skylands/ground/blow/silver_summoner/display_summon_loop
