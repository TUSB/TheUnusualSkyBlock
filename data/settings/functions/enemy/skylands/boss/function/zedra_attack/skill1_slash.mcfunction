...
execute as @e[tag=ZedraFirst,scores={OhMyDatID=0}] run tag @s remove Protection
execute as @e[tag=ZedraFirst] run scoreboard players operation @s OhMyDatID += _ _
## RageAttackの引き継ぎ
execute as @e[tag=ZedraFirst,tag=NowTarget] run function #oh_my_dat:please
data modify storage mob_data: Call.Additional set from storage oh_my_dat: _[-4][-4][-4][-4][-4][-4][-4][-4].AdditionalAI.RageAttack
execute store result score _ _ run scoreboard players get @s OhMyDatID
...
data modify storage oh_my_dat: _[-4][-4][-4][-4][-4][-4][-4][-4].AdditionalAI.RageAttack set from storage mob_data: Call.Additional
## リセット
