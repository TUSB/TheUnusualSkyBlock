...
item replace entity @e[distance=..3,tag=ZedraHead,tag=NowTarget] armor.head with stick{CustomModelData:2005}
data modify entity @e[distance=..3,tag=ZedraHead,tag=NowTarget,limit=1] Pose set value {Head:[0.01f,0f,0f]}
data modify entity @e[distance=..3,tag=ZedraBody,tag=NowTarget,limit=1] Pose set value {RightArm:[40f,0f,25f],LeftArm:[40f,0f,335f],Head:[20f,0f,0f]}
## 移動先を見る（ポーズも合わせる）
