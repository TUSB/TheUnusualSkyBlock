### 即時ステータス
data modify storage tusb_mob: "即時ステータス"."ベース" set value {id:"minecraft:armor_stand",Tags:[Mob,Enemy,DelayedData,HasAI,CallOnInit,CallOnDamage,TickingRequired,AnalyseLog,"sm.KeepPose","Trap"],CustomName:'{"translate":"ファイアーバーの床","color":"#FF8000","bold":true}',Silent:true,Invulnerable:true,ShowArms:false,Invisible:true,Small:true,NoBasePlate:true,DisabledSlots:4144959,PortalCooldown:3000}
...
data modify storage tusb_mob: "遅延ステータス"."ステータス".AnalyseLog set value '[{"translate":"%1$s敵味方問わずチクっとしますよー","with":[{"translate":"【能力：物理無効・反撃】","color":"#00FF00"}]}]'
# AI
