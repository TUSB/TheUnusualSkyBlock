### 即時ステータス
data modify storage tusb_mob: "即時ステータス"."ベース" set value {id:"minecraft:item",NoGravity:1b,Age:100,PickupDelay:32767,PortalCooldown:100,DeathLootTable:"empty",Tags:[DelayedData,HasAI,CallOnDeath,CallOnInit,CallOnTimeOut,CooldownRequired]}
data modify storage tusb_mob: "即時ステータス"."見た目" set value {Item:{id:"minecraft:cobweb",Count:1b}}
# AI
data modify storage tusb_mob: "遅延ステータス".AI set value {Initial:[{Name:"Step",Target:{Look:"player",Radius:16},Speed:0.5d,SpeedRange:1.2d}],Death:[{Name:"Spawn",SpawnEntities:[[{Tags:[Underworld,Event,Function,SpiderReinforce,SpiderwebEffect],Level:1}]],Target:{Look:"player",Radius:1.0d,IgnoreBlocks:true},Speed:0.2d,Condition:[{Name:"HasTarget",Target:{Look:"player",Radius:2}}]}],Time:[{Name:"Kill"}],Turn:[{Skill:[{Interval:{Min:1,Max:1,Current:1},Call:[{Name:"Kill",Condition:[{Name:"HasTarget",Target:{Look:"player",Radius:2}}]}]}],Exit:{Loop:{Max:1,Current:1}}}]}
#Function
particle minecraft:item cobweb ~ ~ ~ 0.1 1 0.1 0.1 10 force @a
playsound entity.spider.hurt master @a[distance=..20] ~ ~ ~ 1.0 2.0
playsound entity.wither.shoot master @a[distance=..20] ~ ~ ~ 1.0 2.0
