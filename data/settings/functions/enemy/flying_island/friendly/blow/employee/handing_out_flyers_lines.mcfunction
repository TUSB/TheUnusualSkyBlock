...
# 目が合ったら
data modify storage mob_data: CCHelper.Insight.texts set value ["ガリッ・・・ガリッ・・・。","(飴玉をガリガリと食べているようだ。)"]
...
# 話し掛けたら
data modify storage mob_data: CCHelper.Interact.texts set value ["ゴクッ・・・ゴクッ・・・！","(ブラックコーヒーを飲んでいる)"]
