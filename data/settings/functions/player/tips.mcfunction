...
data modify storage tusb_player: TIPS append value '[{"with":[{"translate":"ドクロのトラップアイテム等はシュルカーチェストか、バンドルに入れておけば安全に保管出来、また敵にも大ダメージを与えるものも中にはあります。"}],"translate":"[TIPS] %1$s","bold":true},{"translate":"➡このTIPSを表示しない","clickEvent":{"action":"run_command","value":"/trigger TipsSupTrigger set 7"}}]'
