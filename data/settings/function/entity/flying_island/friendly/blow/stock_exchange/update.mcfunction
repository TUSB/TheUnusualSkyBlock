#Function
tellraw @a[nbt={Inventory:[{tag:{Stock:{Status:"normal"}}}]}] [" --- ",{"translate":"証券取引情報"}," --- "]
...
execute unless score _ Calc matches 0 run function settings:entity/flying_island/friendly/blow/stock_exchange/add_stock
...
execute in area:control_area run data modify block 2 3 2 front_text.messages[0] set value '[{"translate":"取引番号："},{"score":{"name":"_","objective":"_"}}]'
execute in area:control_area run data modify storage mob_data: StockVillager.Recipes[-1].sell.tag.display.Lore[-1] set from block 2 3 2 front_text.messages[0]
...
function settings:entity/flying_island/friendly/blow/stock_exchange/exchange_fluctuation
...
execute if data storage mob_data: StockVillager.Exchanged[-2] as @a if data entity @s Inventory[{tag:{Stock:{Status:"normal"}}}] at @s run playsound minecraft:item.goat_horn.sound.1 voice @s ~ ~ ~ 0.2
...
tellraw @a[nbt={Inventory:[{tag:{Stock:{Status:"normal"}}}]}] " ---------------- "
