...
# ループ処理開始
    execute at @s if entity @a[predicate=entity:player,distance=..16] positioned ^ ^ ^0.25 if block ^ ^ ^ #block:no_collision run function settings:entity/global/sky/shoot/crimson_eye/blaze_third_loop
# 当たったプレイヤーにダメージを与える（確定火だるま）
    execute as @a[tag=Landing,distance=..24] run function ai:skill/laser/main/damage
