...
# 演出
    ...
    particle dust{color:[1.0,1.0,100000000.0],scale:1} ~ ~2 ~ 0.5 2 0.5 0 100 force @a[distance=..32,tag=ShowParticles]
    particle item{item:{id:"ice"}} ~ ~ ~ 0 0 0 0.75 20 force @a[distance=..32,tag=ShowParticles]
# 攻撃効果
    data modify storage mob_data: MobName set from entity @s CustomName
    execute as @e[distance=..1.5,predicate=entity:friendly,sort=nearest,limit=1] if entity @s[type=player] run function settings:entity/global/sky/shoot/sherbet_eye/attack2
    execute as @e[distance=..1.5,predicate=entity:friendly,sort=nearest,limit=1] unless entity @s[type=player] run function settings:entity/global/sky/shoot/sherbet_eye/attack3
# Killする
