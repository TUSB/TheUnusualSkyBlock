...
# 対象の最大HPの15%を確定で与える
    ...
    data modify storage score_damage: Argument set value {BypassArmor:1b,DeathCause:'[{"translate":"%1$sは%2$sによって空の塵となった。","with":[{"selector":"@s"},{"storage":"mob_data:","nbt":"MobName","interpret":true}]}]'}
    execute store result storage score_damage: Argument.Damage int 0.15 run attribute @s max_health get 1
