...
# ループ処理開始
    execute at @s if entity @a[predicate=entity:player,distance=..12] positioned ^ ^ ^0.5 run function settings:entity/global/sky/shoot/lunatic_eye/laser_start_loop
# 当たったプレイヤーにダメージを与える（確定混乱）
    execute as @a[tag=Landing,distance=..16] run function ai:skill/laser/main/damage
