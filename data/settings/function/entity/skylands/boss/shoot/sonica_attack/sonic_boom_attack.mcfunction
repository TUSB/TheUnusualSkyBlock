...
# 演出
    ...
    execute at 0-0-0-0-0 run function settings:entity/skylands/boss/shoot/sonica_attack/sonic_boom_attack_loop
# Marker返却
    execute as 0-0-0-0-0 run function calc:geometry/return_marker
# 当たったプレイヤーにダメージを与える（確定麻痺）
    execute as @a[tag=Landing,distance=..32] run function ai:skill/laser/main/damage
    effect give @a[tag=Landing,distance=..32] invisibility 10 5
    execute as @e[type=!player,predicate=entity:friendly,tag=Landing,distance=..32] run function settings:entity/skylands/boss/shoot/sonica_ai/wave_damage3
