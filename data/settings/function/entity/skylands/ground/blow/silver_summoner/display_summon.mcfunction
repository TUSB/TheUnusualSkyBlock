...
# シルバー召喚
    ...
    execute if block ~ ~ ~ #block:no_collision as @e[tag=Spawn,tag=!CallSpawned,distance=0] run function ai:skill/spawn/apply
    execute unless block ~ ~ ~ #block:no_collision as @e[tag=Spawn,tag=!CallSpawned,distance=0] run function settings:entity/skylands/ground/blow/silver_summoner/display_summon_loop
