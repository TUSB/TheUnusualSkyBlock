...
execute as 0-0-0-0-0 unless score @s LeftwardGravity matches 12345 run me function assert_manager:behaviour/reader/moves/leftward/acceleration/set 1

execute as 0-0-0-0-0 run function finalization_manager:kill_immidiately
summon husk ~ ~ ~ {Team:Assert,UUIDMost:0l,UUIDLeast:0l,Tags:[Enemy],Passengers:[{id:item_frame,Tags:[Parameter],Item:{id:stone,Count:1b,tag:{MoveSettingsList:[{Leftward:{Acceleration:1.2345d}}]}}}]}
