playsound minecraft:block.enchantment_table.use master @a ~ ~ ~ 1 0.0
playsound minecraft:block.enchantment_table.use master @a ~ ~ ~ 1 0.5
playsound minecraft:block.enchantment_table.use master @a ~ ~ ~ 1 1.0
playsound minecraft:block.enchantment_table.use master @a ~ ~ ~ 1 1.5
playsound minecraft:block.enchantment_table.use master @a ~ ~ ~ 1 2.0
particle minecraft:portal ~ ~1 ~ 0 0 0 2 50 force
