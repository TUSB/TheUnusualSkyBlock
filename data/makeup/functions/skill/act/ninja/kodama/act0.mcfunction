...
execute if score _ Level matches 1 positioned as @e[distance=..20,type=item,nbt={Invulnerable:false}] run particle minecraft:portal ~ ~ ~ 0 0 0 0.2 50 force @a[tag=ShowParticles]
execute if score _ Level matches 2 positioned as @e[distance=..30,type=item,nbt={Invulnerable:false}] run particle minecraft:portal ~ ~ ~ 0 0 0 0.2 50 force @a[tag=ShowParticles]
