...
### プレイヤー側をターゲット
execute if entity @s[tag=Target.Ally] as @e[tag=Ally,tag=MayTargeted,sort=nearest] run function behaviour_manager:behaviour_applier/target/searcher/check_target/necessity
### 敵側をターゲット
execute if entity @s[tag=Target.Enemy] as @e[tag=Enemy,tag=MayTargeted,sort=nearest] run function behaviour_manager:behaviour_applier/target/searcher/check_target/necessity
