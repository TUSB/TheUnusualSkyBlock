...
#やけどダメージ付与
execute unless block ~ ~ ~ water run data modify storage score_damage: Argument set value {Damage:6,DamageType:[Fire],DeathCause:'[{"translate":"%1$sの効果で焼け死んだ。","with":[{"interpret":true,"storage":"effects:","nbt":"BadEffectsName.Burn"}]}]'}
