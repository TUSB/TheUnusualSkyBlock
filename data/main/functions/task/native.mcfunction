...
###ジン・スピリットコウモリ処理
execute if entity @s[tag=GinSpiritBat] positioned ~-0.5 ~-0.5 ~-0.5 if entity @a[dx=0,dy=0,dz=0,gamemode=!creative,gamemode=!spectator,limit=1] run function skill_manager:black_mage/gin_spirit/heal
###ウィンドウォール竜巻処理
execute if entity @s[tag=WindWallTornado] run function skill_manager:black_mage/wind_wall/tick_tornado
###金タライ処理
