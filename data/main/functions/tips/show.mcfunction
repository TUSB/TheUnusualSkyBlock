...
execute if score $Digit Global matches 5 run tellraw @s ["",{"translate":"[TIPS] ３秒間スニークした状態でジャンプすると、%1$sバーストゲージを消費して、特殊なオーラを纏うことができます。","with":[{"text":"バーストブレイク！！","color":"gold","italic":true,"bold":true}],"bold":true},{"text":"➡このTIPSを表示しない","clickEvent":{"action":"run_command","value":"/trigger TipsSupTrigger set 5"}}]
...
execute if score $Digit Global matches 7 run tellraw @s ["",{"text":"[TIPS] 10秒間スニークすると、プレイヤー情報の表示や設定ができます。","bold":true},{"text":"➡このTIPSを表示しない","clickEvent":{"action":"run_command","value":"/trigger TipsSupTrigger set 6"}}]
