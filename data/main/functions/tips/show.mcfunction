...
execute if score $Digit Global matches 5 run tellraw @s ["",{"translate":"[TIPS] ３秒間スニークした後、画面に表示される数字を入力すると、%1$sバーストゲージを消費して、特殊なオーラを纏うことができます。","with":[{"text":"バーストブレイク！！","color":"gold","italic":true,"bold":true}],"bold":true},{"text":"➡このTIPSを表示しない","clickEvent":{"action":"run_command","value":"/trigger TipsSupTrigger set 5"}}]
