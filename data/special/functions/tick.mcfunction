...
### モブ行動フロー
function behaviour_manager:common/check_all
...
# ### 特殊床
# execute as @a[gamemode=!spectator,gamemode=!creative] unless score @s Levitate matches 0.. positioned as @s if block ~ ~-2 ~ #main:unique_floors run function entity_manager:unique_floor/fork
# ##################################################     非エリア依存ブロック参照処理の壁     ##################################################
...
# ###潜在能力
# execute as @a unless score @s PotentialTrigger matches 0 at @s run function main:settings/potential/triggered
# ##################################################     スキル表示・設定変更の壁     ##################################################
