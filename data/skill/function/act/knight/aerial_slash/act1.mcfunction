...
# 投てき物を召喚
summon arrow ^ ^ ^ {Tags:[Skill,AerialSlash,NativeTask,FlyingRequired,KillInGround,IgnoreReflection],NoGravity:1b,damage:1}
...
# 追尾する敵を特定
execute store result score @e[tag=AerialSlash,tag=!Initialized,distance=..0.01] TrackingID at 0-0-0-0-0 store result score @n[tag=Mob,distance=..0.01] TrackingID run scoreboard players add $World TrackingID 1
