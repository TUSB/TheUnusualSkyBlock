...
execute positioned as @e[tag=SummonedBlock,tag=!Initialized,distance=..1] run setblock ~ ~ ~ minecraft:ender_chest keep
#持続時間 = (300 - Pos[1]) * Level
execute store result score _ Calc run data get entity @s Pos[1] -1
scoreboard players add _ Calc 300
execute store result entity @e[tag=SummonedBlock,tag=!Initialized,distance=..1,limit=1,sort=nearest] PortalCooldown int 1 run scoreboard players operation _ Calc *= _ Level
#演出
