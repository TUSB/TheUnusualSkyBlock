...
#投射物を召喚
execute anchored eyes run summon arrow ^ ^ ^0.5 {Tags:[Skill,ShingingBolt,NativeTask,FlyingRequired,KillInGround],NoGravity:1b,damage:1,Color:15897343}
execute as @e[tag=ShingingBolt,tag=!Initialized,distance=..3] positioned 0.0 0.0 0.0 positioned ^ ^ ^1.55 run function calc:throw_projectile/
#ダメージを保存
execute as @e[tag=ShingingBolt,tag=!Initialized,distance=..3] run function skill:damage/save
#Ownerを設定
data modify entity @e[tag=ShingingBolt,tag=!Initialized,distance=..3,limit=1] Owner set from entity @s UUID
#演出
