...
##召喚士
    ...
    execute if score _ Job matches 3 run tellraw @a ["",{"translate":"%1$sの%2$sの効果が切れた。","color":"light_purple","with":[{"text":"Player","obfuscated":true},{"translate":"狩人のオーラ","hoverEvent":{"action":"show_text","contents":{"translate":"範囲内のプレイヤーを自動砲台にする。"}}}]}]
    execute if score _ Job matches 4 run tellraw @a ["",{"translate":"%1$sの%2$sの効果が切れた。","color":"light_purple","with":[{"text":"Player","obfuscated":true},{"translate":"白魔導士のオーラ","hoverEvent":{"action":"show_text","contents":{"translate":"範囲内のプレイヤーに悪い効果がかからなくなる。"}}}]}]
